...
export class ExpressionParser {
  ...
  private scanPathOrIdentifier(scanner: Scanner): string {
    ...
    while (!scanner.isAtEnd()) {
      ...
      if (this.isAlnum(c) || c === '/' || c === '.' || c === '_' || c === '-' || c === '@' || c === '~' || /[\p{XID_Continue}]/u.test(c)) {
        scanner.advance();
      } else {
        break;
      }
  ...
  private scanIdentifier(scanner: Scanner): string {
    const start = scanner.pos;
    while (!scanner.isAtEnd() && (this.isAlnum(scanner.peek()) || scanner.peek() === '_' || /[\p{XID_Continue}]/u.test(scanner.peek()))) {
      scanner.advance();
    }
    return scanner.input.substring(start, scanner.pos);
  }
