...
import { Bot, Link2, Plus, Trash2, X } from 'lucide-react'
...
export function AgentDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const refresh = () => getAgents().then(setAgents).catch(() => setAgents([]))
  ...
  return <aside className={`drawer ${open ? 'open' : ''}`} aria-hidden={!open}>
    ...
    <div className="connect-box"><Link2 size={17} /><input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://agent.example.com" /><button disabled={busy || !url} onClick={() => void connect()}><Plus size={16} /> Connect</button></div>
